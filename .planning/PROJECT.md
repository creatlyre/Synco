...
- v5.1 E2E Verification & Brand Marketing milestone shipped 2026-03-25 (Phases 36-50).
- 126 Playwright E2E tests passing across 3 user roles (free, pro, family-plus) covering auth, calendar, dashboard, notifications, gating, billing, shopping, sync, and error handling.
- Landing page: benefit-driven copy, comparison table, founder story, GitHub badge, social proof trust indicators, OG/Twitter meta.
- SEO: robots.txt, sitemap.xml, canonical URLs, JSON-LD structured data.
- Brand guide (BRAND.md), CSS custom properties, visual identity cleanup.
- Landing UX: mobile hamburger menu, nav Features link, contrast fixes.
- Pricing tiers: benefit-oriented copy, currency i18n (PLN/EUR), annual toggle.
- Primary CTA points to /auth/register (conversion-focused funnel).
...
- 593 backend tests + 126 E2E tests passing.
- Planning artifacts for v1.0–v5.1 archived under `.planning/milestones/`.
...
- ✓ Social proof trust indicators on landing page (HERO-01) — v5.0
- ✓ Open Graph/Twitter Card meta tags for link sharing (HERO-02) — v5.0
- ✓ Primary CTA points to /auth/register for conversion (HERO-03) — v5.0
- ✓ Benefit-driven feature copy in Polish/English (HERO-04) — v5.0

- ✓ Playwright E2E test infrastructure with multi-role auth fixtures — v5.1
- ✓ 126 E2E tests covering auth, calendar, dashboard, notifications, gating, billing, shopping, sync, error handling — v5.1
- ✓ Entitlement gating verification (free vs pro vs family-plus) — v5.1
- ✓ Landing page copy overhaul with comparison table and founder story — v5.1
- ✓ SEO: robots.txt, sitemap.xml, canonical URLs, JSON-LD structured data — v5.1
- ✓ Brand guide, CSS custom properties extraction, visual identity cleanup — v5.1
- ✓ Pricing tier copy with currency i18n and annual toggle — v5.1
...
- Email/password authentication with login/register pages (AUTH-01, AUTH-02)
- Admin dashboard with user management and plan assignment (ADM-01–04)
...
| CTA /auth/login → /auth/register | Conversion-focused funnel: register is the primary action | ✓ Shipped v5.0 |
| 303 See Other for POST logout | POST-redirect-GET pattern prevents browser resubmission dialog | ✓ Shipped v5.0 |
| logo-wordmark.webp as OG image | Branded social media preview for link sharing | ✓ Shipped v5.0 |
| Playwright with 3 auth projects | Test free/pro/family-plus roles in parallel across full app | ✓ Shipped v5.1 |
| formatReminderMinutes before content block | Script ordering fix — function must be defined before template blocks call it | ✓ Shipped v5.1 |
| Smart discuss for infrastructure phases | Skip grey-area discussion for pure infrastructure, write minimal CONTEXT.md | ✓ Shipped v5.1 |

---
*Last updated: 2026-03-25 after v5.1 milestone*
